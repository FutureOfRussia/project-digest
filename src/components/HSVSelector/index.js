...
import Animated, {
  call, color, cond, eq, useCode, Value,
} from 'react-native-reanimated'
import { State } from 'react-native-gesture-handler'
import {
  hsv2color, hsv2rgb, ReText, string,
} from 'react-native-redash'
import { px } from '../../helpers/Dimensions'
import { Picker, Slider } from './components'
import styles from './styles'

export default function HSVSelector({ onSave }: { onSave: Function }) {
  const hue = new Animated.Value(0.0)
  const saturation = new Value(0.0)
  const value = new Value(1.0)
